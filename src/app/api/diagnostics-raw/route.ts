...
export async function GET() {
  try {
    ...
    try {
      await dbRaw.prisma.$queryRaw`SELECT 1`;
      diagnostics.database.connected = true;

      // Test all tables using raw queries
      const tables = ['telegram_users', 'bot_configurations', 'obs_connections', 'scenes', 'sources', 'command_histories', 'stream_sessions'];
      for (const table of tables) {
        try {
          const result = await dbRaw.prisma.$queryRawUnsafe(`SELECT COUNT(*) as count FROM "${table}"`);
    ...
    // Test Bot Configuration using raw query
    try {
    ...
    // Test OBS Manager using raw queries
    try {
      const obsManager = OBSManager.getInstance();
      diagnostics.obs.managerInitialized = true;

      // Get connections from database using raw query
      const connections = await dbRaw.getOBSConnections();

      diagnostics.obs.connections = connections.map((conn: any) => ({
        id: conn.id,
        name: conn.name,
        host: conn.host,
        port: conn.port,
